...
/**
 * Converts a snippet of markdown answer text into a clean plain-text string
 * suitable for FAQPage structured data.
 *
 * @param {string} md - Raw markdown fragment
 * @returns {string} Plain text
 */
function markdownToPlainText(md) {
  return md
    // Custom [LINK: text] / [AFFILIATE: text] patterns -> display text
    .replace(/\[(?:LINK|AFFILIATE):\s*([^\]]+)\]/g, "$1")
    ...
    .replace(/^\s*[-*]\s+/gm, "")
    // Brand replacement
    .replace(/WormGarden/g, "CompostHeaven")
    // Collapse whitespace
    .replace(/\s+/g, " ")
    .trim();
}
...
/**
 * Extracts question/answer pairs from an article's "Frequently Asked Questions"
 * section. Returns an empty array when the article has no FAQ section.
 *
 * FAQ format expected:
 ...
 *   Answer paragraph(s)...
 *
 * @param {string} content - Full markdown content of an article
 * @returns {Array<{question: string, answer: string}>}
 */
export function getArticleFaqs(content) {
  if (!content) return [];

  // Isolate the FAQ section: from the FAQ heading to the next H2 or end.
  ...
  // Split into H3 question blocks.
  const blocks = section.split(/\n###\s+/).slice(1);

  return blocks
    .map((block) => {
      const [questionLine, ...answerLines] = block.split("\n");
      const question = markdownToPlainText(questionLine);
      // Drop horizontal-rule separators and trailing empties from the answer.
